...
from birds.utilities.NWalign import code_with_modified_residues
...
def pdbbundle2seq(
    tarball_name="pdb-bundle.tar.gz", PERMISSIVE="MSE", outfmt="PDB", allowX=True
):
    ...
    for pdb_bundle_name in names:
        ...
        if outfmt == "PDB":
            header_list += [chain_id_mapping[h] for h in header_list_tmp]
...
def pdb2seq(
    infile="pdb.pdb", PERMISSIVE="MSE", outfmt="PDB", allowX=True, SEQRES=False
):
...
def mmCIF2seq(infile="pdb.cif", PERMISSIVE="MSE", outfmt="PDB", allowX=True):
    ...
    for line in txt.splitlines():
        ...
        if label_atom_id != "CA" or label_alt_id not in [".", "A"]:  # just CA
            continue
...
def pdbtxt2seq(
    txt="", infile="pdb.pdb", PERMISSIVE="MSE", outfmt="PDB", allowX=True, SEQRES=False
):
    ...
    for line in txt.splitlines():
        ...
        if PERMISSIVE == "ATOM" and line[0:6] != "ATOM  ":
            ...
            continue
        elif PERMISSIVE == "MSE" and (
            not line[0:6] == "ATOM  "
            and not (line[0:6] == "HETATM" and residue == "MSE")
        ):
            continue
        ...
        if residue_tuple not in chain_dict[chain_id]:
...
if __name__ == "__main__":
    ...
    for pdb in argv:
        sys.stdout.write(
            pdb2fasta(
                pdb, PERMISSIVE=PERMISSIVE, outfmt=outfmt, allowX=allowX, SEQRES=SEQRES
            )
        )
